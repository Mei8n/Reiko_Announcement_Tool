...
public class menu_takenoexp_take extends Activity {
    ...
    public void onClick(View v) {
        ...
        else if(v.getId() == R.id.takenoexp_take_pl_button1){
            p = MediaPlayer.create(getApplicationContext(), R.raw.an_101a0); //定義
            p.start(); //再生
        }

        else if(v.getId() == R.id.takenoexp_take_pl_button2){
            p = MediaPlayer.create(getApplicationContext(), R.raw.an_101a1); //定義
            p.start(); //再生
        }

        else if(v.getId() == R.id.takenoexp_take_pl_button3){
            p = MediaPlayer.create(getApplicationContext(), R.raw.an_101a2); //定義
            p.start(); //再生
        }

        else if (v.getId() == R.id.takenoexp_take_one_back){
